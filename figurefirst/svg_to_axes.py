...
SCALE_FACTORS = {
    "px": {"in": 72.0, "cm": 72 / 2.54, "mm": 72 / 25.4, "px": 1.0,},
    "in": {"in": 1.0, "cm": 0.3937, "mm": 0.0393, "px": 1 / 72.0,},
    "mm": {"in": 25.4, "cm": 10.0, "mm": 1, "px": 25.4 / 72,},
    "cm": {"in": 2.54, "cm": 1.0, "mm": 0.1, "px": 2.54 / 72,},
}
...
def get_elements_by_attr(xml_node, attribute, value):
        ...
        if xml_node.attributes:
            ...
            if attribute in test_list:
                if xml_node.getAttribute(attribute) == value:
                    yield xml_node
            nls = [
                recur_get_element_by_attr(nd, attribute, value)
                for nd in xml_node.childNodes
            ]
    ...
    return list(
        flatten([el for el in recur_get_element_by_attr(xml_node, attribute, value)])
    )
...
def parse_transform(transform_str):
    ...
    if "translate" in transform_str:
        ...
        if type(txy) is list and len(txy) == 2:
            ...
            tx = txy[0]
            ty = txy[0]
        tr = np.array([[1.0, 0, float(tx)], [0, 1.0, float(ty)], [0, 0, 1.0]])
    ...
    if "scale" in transform_str:
        ...
        sc = np.array([[float(sx), 0, 0], [0, float(sy), 0], [0, 0, 1.0]])
    ...
    if "matrix" in transform_str:
        ...
        mt = np.array([[a, c, e], [b, d, f], [0, 0, 1.0]])
...
class FFSVGGroup(FFItem, object):
    ...
    def __getattr__(self, attr):
        try:
            pnts = np.vstack(
                [
                    np.array([np.array([i.x, i.y]), np.array([i.x + i.w, i.y + i.h])])
                    for i in self.values()
                ]
            )
        except ValueError:
            raise NameError(
                "Could not find an axis for this figure. You probably have a figurefirst:figure tag with no axes associated with it."
            )
...
class FFAxis(FFItem):
    ...
    def __init__(self, tagnode, **kwargs):
        ...
        self.p2 = np.array([x + w, h + y, 1])
        self.record = (
            False  # if this is true, it will save all matplotlib calls to the datafile
        )
                        ...
                        def mpl_wrapper(*args, **kwargs):
                            ...
                            if not regenerate.__is_mpl_call_saveable__(function):
                                s = (
                                    "Function name: "
                                    + function
                                    + " cannot be saved. Try finding a replacement in figurefirst.mpl_functions, or define your own custom wrapper"
                                )
                                raise ValueError(s)
                            regenerate.__save_fifidata__(
                                data_filename,
                            ...
                            self["axis"].__getattribute__(attr[1:])(
                                *args[1:], **kwargs
                            )  # This calls a matplotlib method
                        ...
                        if not self.record:
                            ...
                            if attr in mpl_functions.__dict__.keys():
                                ...
                                def wrapper(*args, **kwargs):
                                    ...
                                    f = regenerate.__load_custom_function__(
                                        package, function
                                    )
                                ...
                                def wrapper(*args, **kwargs):
                                    ...
                                    function = attr
                                    args_description = []
                                    if not regenerate.__is_mpl_call_saveable__(
                                        function
                                    ):
                                        s = (
                                            "Function name: "
                                            + function
                                            + " cannot be saved. Try finding a replacement in figurefirst.mpl_functions, or define your own custom wrapper, or turn off record."
                                        )
                                        raise ValueError(s)
                                    regenerate.__save_fifidata__(
                                        data_filename,
                                    ...
                                    self["axis"].__getattribute__(attr)(
                                        *args, **kwargs
                                    )  # This calls a matplotlib method
...
class PathSpec(dict):
        ...
        for arg in args:
            try:
                if arg.tagName == "figurefirst:pathspec":
                    self.load(arg)
                    self.name = arg.getAttributeNS(
                        "http://flyranch.github.io/figurefirst/", "name"
                    )
    ...
    def load(self, ptag):
        ...
        self.style = dict()
        [
            self.style.update({x.split(":")[0]: x.split(":")[1]})
            for x in self["style"].split(";")
        ]
...
class FigureLayout(object):
    def __init__(
        ...
        self.layout_user_sx = (
            self.layout_uw / self.layout_width[0],
            self.layout_width[1],
        )
        self.layout_user_sy = (
            self.layout_uh / self.layout_height[0],
        ...
        try:
            ...
            print(
                "Warning: key error - probably loading a layout with missing links, like missing figurefirst:template targets."
            )
        # add the spinespecs
        ...
        if (
            np.abs(self.layout_user_sx[0] - self.layout_user_sy[0])
            > figurefirst_user_parameters.rounding_tolerance
        ):
            warn(
                """The the scaling of the user units in x and y are different and may result in unexpected
    ...
    def make_group_tree(self):
            ...
            for child in node.childNodes:
                if child.nodeType == 1:
                    ...
                    if child.tagName in ["figurefirst:axis"]:
                        ...
                        mpl_methods_elements = node.getElementsByTagName(
                        ...
                        for mpl_methods_element in mpl_methods_elements:
                            [
                                grouptree[ax.name].mplmethods.update({key: value})
                                for key, value in mpl_methods_element.attributes.items()
                            ]
                        projection = node.getElementsByTagName("figurefirst:projection")
                        if child.hasAttribute("figurefirst:projection"):
                            grouptree[ax.name].projection = child.getAttribute(
                                "figurefirst:projection"
                            )
                        else:
                            grouptree[ax.name].projection = "rectilinear"
                        if child.hasAttribute("figurefirst:spinespec"):
                            grouptree[ax.name].spinespec = child.getAttribute(
        ...
        for leafname, leaf in leafs.items():
            ...
            tp1 = np.min(sp, axis=0)
            tp2 = np.max(sp, axis=0)
            leaf.p1 = tp1
            leaf.p2 = tp2
    ...
    def set_layer_visibility(
        self, inkscape_label="Layer 1", vis=True, gid=None,
    ):
        ...
        for l in layers:
            if l.attributes["inkscape:label"].value == inkscape_label:
                try:
                    ...
                    style_str = l.attributes["style"].value
                repl_str = re.sub(
                    r"display:(none|inline)", "display:%s" % (value), style_str
                )
                l.setAttribute("style", repl_str)

    def create_new_targetlayer(self, layer_name):
        new_layer = self.output_xml.createElement("g")
        new_targetlayer = self.output_xml.createElementNS(
            XMLNS, "figurefirst:targetlayer"
        )
        ...
        layers = get_elements_by_attr(output_svg, "inkscape:groupmode", "layer")
        for layer in layers:
            if layer_name == layer.getAttribute(
                "id"
            ) or layer_name == layer.getAttribute("inkscape:label"):
                raise ValueError("Layer with that name already exists!")

        # set default attributes
        attributes = {
            u"style": u"display:inline;stroke-linecap:butt;stroke-linejoin:round",
    ...
    def append_figure_to_layer(
        ...
        if cleartarget:
            try:
                ...
                print(
                    "could not clear target, probably because desired target not found, try specifying fflayername, or use autogenlayers=True"
                )
        ...
        target_layer.setAttribute(
            "transform", "scale(%s,%s)" % (output_scale, output_scale)
        )
        ...
        if save_traceback:
            ...
            self.add_attribute_to_layer(
                fflayername, "figurefirst:traceback", tb_formatted
            )
            time_str = (
            ...
            self.add_attribute_to_layer(
                fflayername, "figurefirst:date-modified", time_str
            )
        if notes:
    ...
    def apply_svg_attrs(self, svg_items_to_update="all"):
        ...
        for svgkey, svgitem in leafs.items():
            if svg_items_to_update != "all":
                if (
                    svgkey[0] not in svg_items_to_update
                ):  # for some reason the svgkey is a tuple
                    continue
            nd = svgitem.node
            ...
            if isinstance(svgitem, FFSVGText):
                ...
                outnd.getElementsByTagName("tspan")[0].setAttribute(
                    "style", svgitem.frmtstyle()
                )
    ...
    def pass_xml(self, gid, key, value):
        ...
        if gid[:12] != "figurefirst:":
            warn(
                """Warning: ID should start with 'figurefirst:'
    ...
    def clear_fflayer(self, fflayername):
        ...
        while (
            removed_children != 0
        ):  # this is not the prettiest way to do this, but it mostly works
            remove = True
            while (
                remove
            ):  # getting a list of children and then removing them 1 by 1 does not work!
