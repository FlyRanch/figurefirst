...
T = 1.0
Delta = T / N
for group_name, group_mean, group_sigma in zip(
    ["A", "B"], [groupA_mean, groupB_mean], [groupA_sigma, groupB_sigma]
):
    data[group_name] = dict()
    for cond_name, cond_effect in zip(
        ...
        for trial in range(10):
            ...
            W[: N + 1] = (
                ...
                + group_mean
                + np.cumsum(
                    np.sqrt(Delta) * np.random.standard_normal(N + 1) * group_sigma
                )
            )
            t = np.linspace(0, T, N + 1)
...
# to remove spines
def kill_spines(ax):
    return fifi.mpl_functions.adjust_spines(
        ax,
        "none",
        spine_locations={},
        smart_bounds=True,
...
for group_name, group in layout.axes_groups["none"].items():
    if not (group_name == "summary"):
        ...
        for cond_name, cond_ax in group.items():
            ...
            cond_ax["axis"].plot(
                np.mean(np.array(data[group_letter][cond_name]).T, axis=1), **kwargs
            )
        ...
        for cond_name, cond_ax in group.items():
            cond_ax["axis"].hist(
                np.array(data[group_letter][cond_name]).ravel(),
                bins=50,
                histtype="stepfilled",
                clip_on=False,
                **kwargs
